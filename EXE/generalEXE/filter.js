...
console.log(filter(numbers, 76));

const graterAndLess = (arr, num1, num2)=>{
    return arr.filter((el)=>{
        return el > num1 && el < num2;
    })
}
console.log(graterAndLess(numbers, 10, 200));
...
const filterRating = (arr, rating)=>{
    return arr.filter((el)=>{
        return el.rating.average > rating;
    });

};

console.log(filterRating(movies,7));
 
const filterRatingsAndRunTime = (arr, rating, runTime) => {
    return arr.filter((movie) => {
      return movie.rating.average > rating && movie.runtime < runTime;
    });
  };
  console.log(filterRatingsAndRunTime(movies, 7, 60));

  const filterRatingsAndRunTimeVersion2 = (arr, rating, runTime) => {
    return arr
      .filter((movie) => {
        return movie.rating.average > rating && movie.runtime < runTime;
      })
  ...
  console.log(filterRatingsAndRunTimeVersion2(movies, 6, 60));

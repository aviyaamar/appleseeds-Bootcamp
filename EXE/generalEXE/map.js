...
const floatArr = floatNumbers.map((el)=>{
...
const movies = [
  ...
  const namesOfMovie = movies.map((el)=>{
      return el.name;
  })
  console.log(namesOfMovie);
  

  const newOBj = movies.map((el)=>{
      return {name: el.name, genres: el.genres};
  });
